...
namespace BallWar {
    ...
    public static class BWConstants {
        ...
        public const int INPUT_SHOT = (1 << 6);
        public const int INPUT_SHOT_DOWN = (1 << 106);
    ...
    public struct BwGame : IGame {
        ...
        public void ParseBallInputs(long inputs, int i, out int shot) {
            shot = (int)(inputs & INPUT_SHOT_DOWN);
        }
        ...
        public void CheckShot(int index, int shot) {
            // If the shot input was pressed, fire
            if (shot != 0) {
                PhysicsController pc = PhysicsController.Instance;
                Color ballColor = (index == 0 ? Color.red : Color.blue);
                pc.ShootBall(ballColor, (sfloat)20.0f, (sfloat)12.0f, new float3((sfloat)50.0f, (sfloat)20.0f, (sfloat)(-50.0f)), new float3((sfloat)(-100.0f), (sfloat)1.0f, (sfloat)100.0f));            
                Debug.Log("Ball shot");
                GGPORunner.LogGame($"{ballColor.ToString()} Ball shot");
            }
        }
